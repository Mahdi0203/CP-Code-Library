...
struct node {
  ...
  node() {}
  node(int x) {       //> problem er need wise information store kora lagbe
    val = x; sum = x;
    sz = 1; lazy = 0;
    ssz = 1; vsz = 0;
    subsum = x; vsum = 0;
...
struct LCT {
  ...
  void pull(int x) {    //> problem er need wise information store kora lagbe, just like SegT pull
    if (!x) return;
  ...
  void push(int x) {    //> problem er need wise information store kora lagbe, just like SegT push
    if (!x) return;
    int &l = t[x].c[0], &r = t[x].c[1];
    if (t[x].flip) {
      swap(l, r); 
    ...
    if (t[x].lazy) {    // shudu ei part change kora jabe
      t[x].val += t[x].lazy;
...
void init_LCT(int st, int n) {
  dfs(st);
  lct = LCT(n + 1);
  for (int i = 1; i <= n + 1; i++) lct.t[i] = node(0);   // initial node value
  for (int i = 1; i <= n; i++) lct.link(par[i], i);      // connect nodes
}
...
void solve ( int cs ){
  ...
  init_LCT(1, n);
   while ( q-- ) {
    ...
    lct.upd(a, b, 1);
    int a_b = lct.query(a, b);
 
    lct.upd(c, d, 1);
    int over_lap = lct.query(a, b) - a_b;
    int c_d = lct.query(c, d) - over_lap;
 
    cout << (a_b + c_d - over_lap) << '\n';
...
// problem : https://codeforces.com/gym/106059/problem/D
